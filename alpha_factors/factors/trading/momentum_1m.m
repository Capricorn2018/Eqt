...
%     tgt_tag  = 'momentum_1m'; % 这里改了下
%     tgt_file = [a.output_data_path,'/','momentum_1m.h5']; % 这里改了下
% 
...
%     end
    ...
    tgt_file = [a.output_data_path,'/momentum_1m.mat'];
    if exist(tgt_file,'file')==2
        momentum_1m = load(tgt_file);
        dt = momentum_1m.data.DATEN;
        dt_max = max(dt);
        bool = true;
    else
        dt_max = 0;
    ...
    if dt_max<p.all_trading_dates(end)
        ...
        if bool
            result = factor_append(momentum_1m,new);
        else
            result = new;
        end
        ...
        data.momentum_1m = average(stk_num,DATEN,key,factor,all_dates,dt_max,len);
