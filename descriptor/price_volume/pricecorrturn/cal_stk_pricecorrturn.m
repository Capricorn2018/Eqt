function  cal_stk_pricecorrturn(p,a,D,if_mix)
   
   T = length(p.all_trading_dates);
   N = length(p.stk_codes);
   tgt_tag  = 'pricecorrturn';  
   tgt_file = [a.output_data_path,'\','pricecorrturn_',num2str(D),'-',num2str(if_mix),'.h5'];

   [S,pricecorrturn] =  check_exist(tgt_file,['/',tgt_tag],p,T,N);
  
   if S>0      
       if if_mix
          vi = sector_pricecorrturn(a,D,S,T,pricecorrturn);
       end
           ...
           for j = 1: N
              if p.all_trading_dates(i)>ipo_dates(j)
                 ...
                 if sum(sus)>length(sus)-2
                     if if_mix
                         pricecorrturn(i,j) = (1-tao*tao*tao)*c + tao*tao*tao*vi(i,j);
                     else
                         pricecorrturn(i,j) = c;
                     end  
                 else
                     if if_mix
                         pricecorrturn(i,j) = vi(i,j);
                     else
                         pricecorrturn(i,j) = NaN;
                     end   
                 end
              end
           end
