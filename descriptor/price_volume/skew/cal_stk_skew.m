function  cal_stk_skew(p,a,D,if_mix)
   
   T = length(p.all_trading_dates);
   N = length(p.stk_codes);
   tgt_tag  = 'tskew';  
   tgt_file = [a.output_data_path,'\','tskew_',num2str(D),'-',num2str(if_mix),'.h5'];


   [S,tskew] =  check_exist(tgt_file,['/',tgt_tag],p,T,N);
  
   if S>0      
       if if_mix
          vi = sector_skew(a,D,S,T,tskew);
       end
           ...
           for j = 1: N
              if p.all_trading_dates(i)>ipo_dates(j)
                ...
                % y(isnan(y))= 0;
                 ...
                 if tao~=1
                     if if_mix
                         tskew(i,j) = (1-tao*tao*tao)*skewness(y) + tao*tao*tao*vi(i,j);
                     else
                         tskew(i,j) = skewness(y);
                     end  
                 else
                     if if_mix
                         tskew(i,j) = vi(i,j);
