function  cal_stk_mv(p,a,D1,D2,if_mix)
   
   T = length(p.all_trading_dates);
   N = length(p.stk_codes);
   tgt_tag  = 'mv';  
   tgt_file = [a.output_data_path,'\','mv_',num2str(D1),'_',num2str(D2),'-',num2str(if_mix),'.h5'];

   [S,mv] =  check_exist(tgt_file,['/',tgt_tag],p,T,N);
  
   if S>0      
       if if_mix
          vi = sector_mv(a,D1,D2,S,T,mv);
       end
       ...
       for  i  = S  : T
          for j = 1: N
              ...
              %   y(isnan(y))= 0;
                 ...
                 if tao==1
                     if if_mix
                         mv(i,j) = (1-tao*tao*tao)*(mean(z-D1+1)/mean(z)) + tao*tao*tao*vi(i,j);
                     else
                         mv(i,j) = mean(z-D1+1)/mean(z);
                     end      
                 else
                     if if_mix
                         mv(i,j) = vi(i,j);
