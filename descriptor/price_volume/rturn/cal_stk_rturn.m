function  cal_stk_rturn(p,a,D,if_mix)
   
   T = length(p.all_trading_dates);
   N = length(p.stk_codes);
   tgt_tag  = 'rturn';  
   tgt_file = [a.output_data_path,'\','rturn_',num2str(D),'-',num2str(if_mix),'.h5'];

   [S,rturn] =  check_exist(tgt_file,['/',tgt_tag],p,T,N);
  
   if S>0      
       if if_mix
          vi = sector_rturn(a,D,S,T,rturn);
       end
       ...
       for  i  = S  : T
           for j = 1: N
              if p.all_trading_dates(i)>ipo_dates(j)
                 ...
                 if tao~=1
                     if if_mix
                         rturn(i,j) = (1-tao*tao*tao)*std(t)/mean(t) + tao*tao*tao*vi(i,j);
                     else
                         rturn(i,j) = std(t)/mean(t);
                     end  
                 else
                     if if_mix
                         rturn(i,j) = vi(i,j);
