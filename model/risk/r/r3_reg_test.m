function  r3_reg_test(p,a,K1,K2)
     warning('off')
...
%%
    ...
    for  idx_index = K1:K2
        ...
        factor_rtn_matrix = NaN(T,S+M+1); % 市场（1） + 行业（M）+ 风格(S)
        ...
        for i = 1 : T
           %    disp(i)
              ...
              %　算X
              T_sector = T_sector_style(:,~ismember(T_sector_style.Properties.VariableNames,p.style));
              T_style  = T_sector_style(:,ismember(T_sector_style.Properties.VariableNames,p.style));
              ...
              pre_reg =  array2table([y,ones(height(T_sector),1),x1(:,ok_x_col_sectors),x2(:,ok_x_col_styles)],'RowNames',T_sector_today.Properties.RowNames,...
                                    'VariableNames', [{'y';'mkt'};T_sector.Properties.VariableNames(ok_x_col_sectors)';p.style(ok_x_col_styles)]);
              
              
              % prepare to regression  
              % X = [1,行业因子，风格因子 ]，先求行业因子    计算基准在各个行业上的分布             
              indexw0 =  index_membs_n(max(i-1,1),idx_stk)/100;  % 所有股票一天的权重情况
              %indexw0 =  index_membs_n(i,idx_stk)/100;
           ...
           %   disp([i, sum(u,1)]);
              ...
              x = [ones(size(x0,1),1),x0(:,ok_x_col_sector),table2array(T_sector_style(:,p.style(ok_x_col_style)))];
              ...
              tbl = array2table([ mtimes(w_,y_),mtimes(w_,x_)],'RowNames',T_sector_today.Properties.RowNames(id_in_reg),...
                                    'VariableNames', [{'y';'mkt'};T_sector.Properties.VariableNames(ok_x_col_sector)';p.style(ok_x_col_style)]);
% 
%               tbl = array2table([ y_,x_(:,1)],'RowNames',T_sector_today.Properties.RowNames(id_in_reg),...
%                                     'VariableNames', {'y';'mkt'});
